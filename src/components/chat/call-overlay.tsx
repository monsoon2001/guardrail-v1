...
export function CallOverlay({ callId, onEnd }: { callId: string; onEnd: () => void }) {
  ...
  // 1. Initial Call State & Termination Watcher
  useEffect(() => {
  ...
  // 2. Hardware & Signaling Initialization
  useEffect(() => {
    ...
    const init = async () => {
      ...
      try {
        ...
        setPermissionError(true);
        toast({ variant: "destructive", title: "Hardware Blocked", description: "Camera or Mic access was denied." });
      }
    };
    ...
    const sync = async () => {
      ...
      if (user.uid === callData.calleeId && callData.offer && p.signalingState === 'stable') {
        ...
        while (iceBuffer.current.length) {
          await p.addIceCandidate(new RTCIceCandidate(iceBuffer.current.shift()!));
        }
        const answer = await p.createAnswer();
        await p.setLocalDescription(answer);
        await updateDoc(doc(db, "calls", callId), { answer: { sdp: answer.sdp, type: answer.type }, status: "active" });
      }
    ...
    if (remoteStream) {
      [remoteVideoRef.current, remoteAudioRef.current].forEach(el => {
        if (el) {
          ...
          el.play().catch(() => setNeedsAutoplayGesture(true));
        }
      });
    }
  }, [remoteStream]);
  ...
  return (
    <div className="fixed inset-0 z-[10000] bg-slate-950 flex flex-col items-center justify-center animate-in fade-in duration-500 overflow-hidden">
      ...
      <div className={`absolute top-8 right-8 w-32 md:w-48 aspect-video bg-slate-900 rounded-2xl overflow-hidden border-2 border-slate-800 shadow-2xl z-20 ${callData?.type !== "video" ? 'hidden' : ''}`}>
